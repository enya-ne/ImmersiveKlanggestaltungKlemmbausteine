                ...
                if (state == fluid_synth_status.FLUID_SYNTH_PLAYING)
                {
                    ...
                    lock (this)
                    {
                        ...
                        while (block < DspBufferSize)
                        {
                            ...
                            {
                                int sampleIndex = block + i;
                                if (channels == 1)
                                {
                                    // Mono: mix left and right channels
                                    if (sampleIndex < data.Length)
                                        data[sampleIndex] = (left_buf[i] + right_buf[i]) * 0.5f * vol;
                                }
                                else if (channels >= 2)
                                {
                                    // Stereo or multi-channel: write left and right to first two channels
                                    int j = sampleIndex * channels;
                                    if (j + 1 < data.Length)
                                    {
                                        data[j] = left_buf[i] * vol;
                                        data[j + 1] = right_buf[i] * vol;
                                    }
                                    // For channels > 2, leave additional channels as zero (they're already cleared)
                                }
                            }
#if DEBUG_TRY_CATCH_AUDIODATA
                                }
